...
| Mounted Dart/framework adapter trace | Pass in the current macOS tree, iPhone 17 Pro Simulator (iOS 26.5), and Android API 36 emulator test processes on 2026-08-22 | The playground mounted a focused platform input client and injected framework callbacks through Flutter's test messenger using debug client id `-1`. Homeric committed a composed Unicode replacement as one undo unit, committed visible composition once on connection loss, reattached with a different focused epoch capability, rejected the retired callback without changing canonical state, and routed `TextInputAction.newline` through one split/retarget/undo unit that accepted the next delta. The current simulator/emulator rerun paired this trace with the mobile touch integration gate and passed 2/2 on each platform. This does not exercise native host delivery or client-id routing. |
