...
/// Journal markdown decorations: hide `` ` `` delimiters, paint inner text as code.
List<Decoration> journalMarkdownDecorationsForBlock(Block block) {
  ...
  for (final match in RegExp(r'`([^`]+)`').allMatches(text)) {
    ...
    styleRange(match.start + 1, match.end - 1, 'code');
  }
...
ParagraphSource<TextStyle> _codeAfterSpaceSource(_JournalPaintMap paintMap) =>
    ParagraphSource.build(
      block: para('b', '`foo` '),
      decorations: journalMarkdownDecorationsForBlock(para('b', '`foo` ')),
      resolveStyle: paintMap.resolve,
    );

void main() {
  testWidgets(
      'after space, hidden backticks fold delimiters and inner text paints as code',
      (tester) async {
    const literal = '`foo` ';
    ...
    expect(
      ...
      'monospace',
      reason: 'code inner text must paint monospaced after hide-on-space',
    );
    ...
    expect(_paragraphRender(tester).source.viewText, 'foo ',
        reason: 'backticks must stay hidden after hide-on-space rebuild');
    expect(paintMap.styleAtViewOffset(0)?.fontFamily, 'monospace',
        reason: 'monospace must survive hide-on-space host rebuilds');
  });

  testWidgets(
      'after hide-on-leave, hidden backticks stay folded at trailing space',
      (tester) async {
    const literal = '`foo` ';
    ...
    expect(_paragraphRender(tester).source.viewText, isNot('foo '),
        reason: 'caret on a hidden backtick must reveal it');
    ...
    expect(_paragraphRender(tester).source.viewText, 'foo ',
        reason:
            'trailing-space caret must not re-reveal backticks (HOM-35 class)');
    expect(paintMap.paintCalls, greaterThan(0),
        reason: 'paintStyler must run when the resolve map is refilled');
    expect(paintMap.styleAtViewOffset(0)?.fontFamily, 'monospace',
  ...
  testWidgets(
      'layout-equal source update reshapes paintStyler glyphs for hidden code',
      (tester) async {
    final paintMap = _JournalPaintMap();
    paintMap.beginBuild();
    final source = _codeAfterSpaceSource(paintMap);
    ...
    paintMap.paintCalls = 0;
    paintMap.paintedStyles.clear();
    render.source = _codeAfterSpaceSource(paintMap);
    await tester.pump();
